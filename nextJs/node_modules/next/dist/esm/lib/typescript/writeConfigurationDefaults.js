...
export async function writeConfigurationDefaults(ts, tsConfigPath, isFirstTimeSetup, isAppDirEnabled, distDir, hasPagesDir) {
    ...
    if (isAppDirEnabled) {
        ...
        if (!userTsConfig.compilerOptions || plugins.length && !hasNextPlugin && "extends" in rawConfig && (!rawConfig.compilerOptions || !rawConfig.compilerOptions.plugins)) {
            Log.info(`\nYour ${chalk.bold("tsconfig.json")} extends another configuration, which means we cannot add the Next.js TypeScript plugin automatically. To improve your development experience, we recommend adding the Next.js plugin (\`${chalk.cyan('"plugins": [{ "name": "next" }]')}\`) manually to your TypeScript configuration. Learn more: https://nextjs.org/docs/app/building-your-application/configuring/typescript#the-typescript-plugin\n`);
    ...
    Log.info("");
    if (isFirstTimeSetup) {
        Log.info(`We detected TypeScript in your project and created a ${chalk.cyan("tsconfig.json")} file for you.`);
        return;
    }
    Log.info(`We detected TypeScript in your project and reconfigured your ${chalk.cyan("tsconfig.json")} file for you. Strict-mode is set to ${chalk.cyan("false")} by default.`);
    if (suggestedActions.length) {
        Log.info(`The following suggested values were added to your ${chalk.cyan("tsconfig.json")}. These values ${chalk.cyan("can be changed")} to fit your project's needs:\n`);
        suggestedActions.forEach((action)=>Log.info(`\t- ${action}`));
        Log.info("");
    }
    if (requiredActions.length) {
        Log.info(`The following ${chalk.white("mandatory changes")} were made to your ${chalk.cyan("tsconfig.json")}:\n`);
