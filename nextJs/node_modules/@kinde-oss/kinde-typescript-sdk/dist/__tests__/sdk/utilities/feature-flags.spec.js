...
describe('feature-flags', () => {
    ...
    describe('getFlag', () => {
        it('throws error if no flag is found no defaultValue is given', async () => {
            ...
            await expect(async () => await getFlag(sessionManager, code)).rejects.toThrowError(new Error(`Flag ${code} was not found, and no default value has been provided`));
        });
        it('throw error if provided type is different from typeof of found flag', async () => {
            ...
            await expect(async () => await getFlag(sessionManager, code, true, 's')).rejects.toThrowError(new Error(`Flag ${code} is of type ${FlagDataType[flag.t]}, expected type is ${FlagDataType.s}`));
        });
