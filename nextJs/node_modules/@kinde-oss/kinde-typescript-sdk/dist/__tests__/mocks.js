export const fetchClient = jest.fn().mockImplementation(async () => await Promise.resolve({
    json: async () => {
        await Promise.resolve();
    },
}));
export const getMockAccessToken = (domain = 'local-testing@kinde.com', isExpired = false) => {
...
export const getMockIdToken = (domain = 'local-testing@kinde.com', isExpired = false) => {
    const iat = Math.floor(Date.now() / 1000);
    const exp = isExpired ? iat : iat + 1000000;
    const tokenPayload = {
